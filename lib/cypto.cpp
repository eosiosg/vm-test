...
public_key::public_key(const signature& sig, const sha256& digest, bool require_canonical) {
    if ( require_canonical && !is_canonical(sig)) {
        cerr << "signature is not canonical." << endl;
...
pair<bool, size_t> public_key::serialize(void* pDstBuffer, const void* pData, size_t bufferSize) {
    auto len = ((vector<char>*)pData)->size();
    if(len + 1 > bufferSize) return make_pair(false, 0);
    memset(pDstBuffer, 0, 1);
...
pair<bool, size_t> public_key::deserialize(void* pDstData, const void* pBuffer, size_t bufferLength) {
    ...
    memcpy(p, (const char*)pBuffer + 1, 33);
