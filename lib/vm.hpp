...
public:
    ...
    void create(eosio::name eos_account, string eth_address) {
        backend_t backend(wasm);
        init_backend(backend);

        auto &&create_acc = create_act{eos_account, std::move(eth_address)};
    ...
    action_result raw(hex_code trx_code, eth_addr sender = "") {
        backend_t backend(wasm);
        init_backend(backend);
